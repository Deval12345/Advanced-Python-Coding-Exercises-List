...
Here we define a class named DBField. Inside it we implement only the special dunder method get. The get method receives the instance and the owner. If the value is not yet in the instance cache, we load it and store it in the cache. Then we return the cached value. Because DBField is a descriptor, when we access the data attribute on a user instance Python calls get. The first access triggers the load; later accesses use the cache. The class stays clean and the lazy behavior is reusable.
