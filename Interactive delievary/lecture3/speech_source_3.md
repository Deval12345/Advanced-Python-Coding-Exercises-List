...
In the previous session, we explored protocols and informal interfaces. We saw that Python decides compatibility based on behavior, not inheritance, and that objects become interchangeable when they implement the required methods.

Today we go one layer deeper. We look at how Python resolves attribute access and how you can intercept it when an attribute is not yet known or not yet loaded.
...
## Section 1 — Why Attribute Interception Exists

### CONCEPT 0.1

In the previous session we saw that protocols and duck typing let objects participate in Python based on behavior. Today we extend that by asking how an object can respond when someone accesses an attribute that does not exist yet on the object.
...
## Section 2 — The special dunder method getattr
...
## Section 3 — Proxy and Virtual Attributes
...
## Section 4 — The special dunder method getattribute
