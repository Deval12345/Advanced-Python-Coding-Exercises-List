...
Writing the function and then reassigning it to the result of the decorator is correct but verbose. Python lets you write at-sign logger above the function definition. That means the function is passed to logger and the result replaces the original name. So at-sign logger above def add is the same as defining add and then reassigning add to logger of add. It is syntactic sugar for applying a decorator at definition time.
